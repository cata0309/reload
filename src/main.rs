...
impl Object {
    ...
    pub fn attack(&mut self, target: &mut Object) {
        ...
        if damage > 0 {
            ...
            println!("{} attacks {} but it has no effect",
...
fn hero_move_or_attack(dx: i32, dy: i32, game: &Game, objects: &mut [Object]) {
...
fn hero_death(hero: &mut Object) {
    println!("you died");
    hero.char = '%';
    hero.color = DARK_RED;
}

fn hostile_death(hostile: &mut Object) {
    println!("{} is dead", hostile.name);
    hostile.char = '%';
...
fn render_all(tcod: &mut Tcod,
    ...
    if let Some(fighter) = objects[HERO].fighter {
        tcod.root.print_ex(
            1,
            SCREEN_HEIGHT - 2,
            BackgroundFlag::None,
...
fn main() {
    ...
    let mut game = Game { plan: gen_plan(&mut objects) };
    ...
    while !tcod.root.window_closed() {
        ...
        let hero_action = handle_input(&mut tcod, &game, &mut objects);
